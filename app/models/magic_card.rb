class MagicCard < ApplicationRecord
  ...
  def perform
        ...
        MagicSet.all.each do |x|
            ...
            scraper.get('https://www.cardkingdom.com/purchasing/mtg_singles/?filter[sort]=name&filter[search]=mtg_advanced') do |search_page|
                form = search_page.form_with(:id => 'search') do |search|   
                    search['filter[rarity][0]']= 'M' 
                    search['filter[rarity][1]'] = 'R'
                    search['filter[ipp]'] = 400 
                    search['filter[category_id]'] = ckSetCode
